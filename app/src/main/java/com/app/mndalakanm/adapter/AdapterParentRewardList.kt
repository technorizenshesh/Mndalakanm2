...
//        modelLogin = sharedPref.getUserDetails(AppConstant.USER_DETAILS)
    ...
    override fun onBindViewHolder(
        ...
        val data: SucessRewardList.Result = transList!!.get(position)
        holder.binding.nameTxt.text = data.name
        holder.binding.timeTxt.text = "+ " + data.time + mContext.getString(R.string.min)
        holder.binding.switchActive.visibility = View.VISIBLE
        holder.binding.rewardBtn.visibility = View.GONE

        if (data.status == "Active") {
            holder.binding.switchActive.isChecked = true
        }
        else {
            holder.binding.parent.background = mContext.getDrawable(R.drawable.desible_bg)
        ...
        holder.binding.rewardBtn.setOnClickListener {
            listener.onClick(position, data, "no", "1")
